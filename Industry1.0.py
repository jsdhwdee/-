...
url_pattern = "https://jobs.51job.com/{}/hy01/p{}/"
cities = ['beijing', 'chengdu', 'ningbo', 'chongqin', 'xian', 'suzhou', 'wuhan', 'nanjing', 'tianjing', 'zhengzhou',
          'changsha', 'dongguan', 'foshan', 'qingdao', 'shenyang', 'guangzhou', 'hangzhou', 'shanghai', 'shenzhen']
...
for city in cities:
        for page in range(1, 51):
            # 增加时延防止反爬虫
            time.sleep(5)
            url = url_pattern.format(city, page)
            print(url)
            ...
            for i in soup.find('div', class_='detlist gbox').find_all('div'):
                ...
                with open(city+'.csv', 'a+', encoding='utf-8-sig') as f:
                    f.write(job + ',' + company + ',' + place + ',' + salary + ',' + date + ',' + education + ',' + experience + ',' + scale + ',' + detail + '\n')